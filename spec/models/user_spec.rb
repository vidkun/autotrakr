...
describe User do
  ...
  it { is_expected.to have_fields(:username, :email, :first_name, :last_name, :password_digest) }
  it { is_expected.to validate_presence_of(:username) }
  it { is_expected.to validate_presence_of(:email) }
  it { is_expected.to validate_presence_of(:password) }
  it { is_expected.to validate_uniqueness_of(:email).case_insensitive.with_message("is already taken") }
  it { is_expected.to validate_uniqueness_of(:username).case_insensitive.with_message("is already taken") }

  it 'fails because no passwrod' do
    expect( User.new( { username: 'bill' } ).save ).to be false
  end
 
  it { is_expected.to validate_length_of(:password).greater_than(8) }
